...
export default function PropertyForm({ config, onChange }) {
  const { units } = config;
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Zillow Reference */}
      <Section title="Zillow Listing Reference" icon="🔗">
        <div className="sm:col-span-2">
          <InputField
            label="Zillow URL (reference only)"
            type="text"
            value={config.zillowUrl}
            onChange={v => setField('zillowUrl', v)}
            hint="Paste the Zillow listing URL here for your records. Enter the financial details manually below."
          />
        </div>
      </Section>
